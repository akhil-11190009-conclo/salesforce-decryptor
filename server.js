    ...
    // In production (Render.com), this will load from Render's config vars.
    ...
    fastify.post('/decrypt', {
      ...
      try {
        ...
        // Assuming PKCS1 padding as 'oaepHashingAlgorithm' is 'NONE' in the sample.
        // The encryptedKey is typically Base64 encoded.
        const decryptedSymmetricKey = crypto.privateDecrypt(
          {
            key: PRIVATE_KEY_PEM,
            padding: crypto.constants.RSA_PKCS1_PADDING, // Common for non-OAEP
          },
        ...
        if (error.code === 'ERR_OSSL_RSA_PRIVKEY_DECRYPT') {
            reply.status(400).send({ error: 'DecryptionFailed', message: 'RSA private key decryption failed. Check key, padding, or encryptedKey format.' });
        } else if (error.code === 'ERR_OSSL_EVP_DECRYPT_NOT_INITIALIZED' || error.code === 'ERR_OSSL_EVP_DECRYPT_FINAL') {
            reply.status(400).send({ error: 'DecryptionFailed', message: 'Symmetric decryption failed. Check key, IV, or encryptedData format.' });
        } else if (error.name === 'SyntaxError') {
            reply.status(400).send({ error: 'InvalidJsonFormat', message: 'Decrypted data is not valid JSON.' });
        } else {
